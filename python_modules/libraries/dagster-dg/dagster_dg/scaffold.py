...
def scaffold_project(
    ...
    use_editable_components_only: Optional[str],
    skip_venv: bool = False,
    populate_cache: bool = True,
) -> None:
    click.echo(f"Creating a Dagster project at {path}.")

    if use_editable_dagster and use_editable_components_only:
        exit_with_error(
            "Cannot specify both --use-editable-dagster and --use-editable-components-only."
        )
    ...
    elif use_editable_components_only:
        editable_dagster_root = (
            _get_editable_dagster_from_env()
            if use_editable_components_only == "TRUE"
            else use_editable_components_only
        )
...
def _get_editable_dagster_from_env() -> str:
    if not os.environ.get("DAGSTER_GIT_REPO_DIR"):
        exit_with_error(
            "The `--use-editable-dagster` and `--use-editable-components-only` options, when used as flags,"
